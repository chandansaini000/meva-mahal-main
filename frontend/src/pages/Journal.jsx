...
export default function Journal() { return <main className="max-w-5xl mx-auto px-6 py-16" data-aos="fade-up"><p className="uppercase tracking-[.2em] text-xs text-clay font-medium mb-4">The journal</p><h1 className="font-display text-5xl mb-10">Notes from the harvest.</h1><div className="grid md:grid-cols-3 gap-6">{posts.map(([title, body], index) => <article key={title} data-aos="fade-up" data-aos-delay={(index % 3) * 100} className="border border-line rounded-xl2 p-6 bg-white/40"><h2 className="font-display text-2xl mb-3">{title}</h2><p className="text-ink/65 leading-relaxed">{body}</p></article>)}</div></main>; }
